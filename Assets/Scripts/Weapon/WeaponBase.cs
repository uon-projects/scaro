...
{
	[Header("Gun Attributes")] public string weaponName;
	...
	[Header("UI Refs")] public RewardText rewardText;
	public Text weaponNameText;
	public Text weaponAmmoText;


	[Header("Animation Attributes")] public Vector3 aimPos;
	public float aimingSpeed = 8f;

	[Header("Game System Attributes")] public FundSystem fundSystem;
	public LevelSystem levelSystem;

	[Header("Sounds")] public AudioClip drySound;
	public AudioClip gunFireSound;


	[Header("External Refs")] public UnityStandardAssets.Characters.FirstPerson.FirstPersonController controller;
	...
	[Header("Internal Refs")] public Transform shootPoint;
	public Transform muzzlePoint;
	public Transform caseSpawnPoint;
	public ParticleSystem muzzleflash;

	[Header("Weapon Upgrades")] public float upgradeDamageFactor = 1;
	...
	void Start()
	{
		...
		{
			_light = flashlightObj.GetComponent<Light>();
		...
		if (laserObj != null)
		{
			laserObj.GetComponent<LaserMachine>().SetLaserActive(isLaserOn);
		}
		...
		for (int i = 0; i < pellets; i++)
		{
			if (Physics.Raycast(shootPoint.position, CalculateSpread(spread, shootPoint), out hit, range))
			{
				...
				else
				{
					...
					GameObject bulletHole = Instantiate(bulletImpact, hit.point,
